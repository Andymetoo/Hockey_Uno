...
function createFallbackEnemyResponseProfile() {
    return {
...
function buildTargetModMap(state, selector) {
    return Object.fromEntries(state.targets.map((target) => [target.id, selector(target)]));
}
function validateCampaignProfile(profile) {
    ...
    const coherentBundle = profile.enemyResponse.codeName === "Aggressive interception"
...
function createCampaignProfile(state, now) {
...
function getCampaignProfileTargetMod(profile, targetId, kind) {
    const source = kind === "priority" ? profile.targetPriorityMods
        : kind === "repair" ? profile.targetRepairMods
            : profile.targetDefenseMods;
    return source[targetId] ?? 1;
}
function applyCampaignProfileToTargets(state) {
    for (const target of state.targets) {
        target.hiddenDefenseLevel = clamp(Math.round(target.hiddenDefenseLevel * getCampaignProfileTargetMod(state.campaign.profile, target.id, "defense")), 18, 100);
    }
}
function getCampaignResolutionState(state) {
    return state.campaign.resolutionState;
}
function isCampaignClosedToOrders(state) {
    return getCampaignResolutionState(state) !== "active";
}
function getCampaignClosedOrderMessage(state) {
    return isCampaignClosedToOrders(state) ? CAMPAIGN_CONCLUDED_MESSAGE : null;
}
function registerOperationalBeat(state, tracker) {
...
function getResolutionPendingWorkSummary(state) {
    const pending = [];
...
function hasPendingResolutionWork(state) {
...
function selectRelevantHooks(state, options = {}) {
...
function createOrRefreshHook(state, input) {
    const existing = state.campaign.causalHooks.find((hook) => hook.hookType === input.hookType
...
function resolveHook(state, hookType, relatedTargetId, consumedById) {
    const hook = state.campaign.causalHooks.find((entry) => entry.hookType === hookType && entry.relatedTargetId === relatedTargetId && entry.state !== "resolved");
    if (!hook) {
        return;
    }
    hook.state = "resolved";
...
function upsertInsight(state, subject, conclusion, status, now) {
...
function addCampaignEvent(state, kind, title, body, now, relatedTargetId = null, relatedHookIds = []) {
...
function maybeCreateOpportunity(state, kind, description, sourceTargetId, relatedTargetId, beneficiaryTargetIds, eligibleOperationTypes, eligibleRouteRisks, hookIds, now, fadesAfterBeatDelta) {
    const existing = state.campaign.opportunities.find((opportunity) => opportunity.kind === kind && opportunity.relatedTargetId === relatedTargetId && opportunity.state !== "resolved");
    if (existing) {
        existing.state = "active";
        existing.fadingStartsDay = Math.min(existing.fadingStartsDay ?? Number.MAX_SAFE_INTEGER, state.campaign.currentDay + Math.max(0, fadesAfterBeatDelta - 1));
        existing.fadingStartsBeat = Math.min(existing.fadingStartsBeat ?? Number.MAX_SAFE_INTEGER, state.campaign.operationalBeat + Math.max(0, fadesAfterBeatDelta - 1));
...
function opportunityExpiryReached(opportunity, state) {
...
function opportunityShouldFade(opportunity, state) {
...
export function isOpportunityUsable(opportunity, state) {
...
export function doesOperationExploitOpportunity(opportunity, targetId, operationType, routeRisk) {
...
export function getPrimaryUsableOpportunity(state) {
    return state.campaign.opportunities.find((opportunity) => isOpportunityUsable(opportunity, state)) ?? null;
}
function recordOpportunityDisposition(state, opportunity, disposition, reason, now) {
    ...
    addCampaignEvent(state, "opportunity", disposition === "exploited"
        ? "Opportunity exploited"
        : disposition === "lost_through_delay"
            ? "Opportunity lost"
            : disposition === "confirmed_not_exploited"
                ? "Opportunity surrendered"
                : "Opportunity resolved", reason, now, opportunity.relatedTargetId, opportunity.relatedHookIds);
}
function markOpportunityUseOnLaunch(state, targetId, operationType, routeRisk, now) {
...
function updateOpportunityStates(state, now, beatDelta, dayAdvanced) {
...
function updateHookLifecycle(state, now, beatDelta, dayAdvanced) {
    if (beatDelta <= 0 && !dayAdvanced) {
        return;
    }
    for (const hook of state.campaign.causalHooks) {
        ...
        const shouldFade = (hook.fadingStartsBeat !== null && state.campaign.operationalBeat >= hook.fadingStartsBeat)
...
function getActiveOpportunityBenefitsForTarget(state, targetId) {
    return state.campaign.opportunities.filter((opportunity) => isOpportunityUsable(opportunity, state)
        && (opportunity.beneficiaryTargetIds.length === 0 || opportunity.beneficiaryTargetIds.includes(targetId)));
}
function buildMissionCampaignContext(state, target, operationType, routeRisk) {
    ...
    const addRisk = (key, amount, reason) => {
        if (amount === 0) {
            return;
        }
        context.riskModifiers.push({ key, amount, reason });
    };
    const addReport = (key, amount, reason) => {
...
function missionContextRiskTotal(context) {
    if (!context) {
        return 0;
    }
    return context.riskModifiers.reduce((sum, modifier) => sum + modifier.amount, 0);
}
function describeContextualReport(stage, baseText, context) {
...
function applyMissionCampaignConsequences(state, mission, target, now) {
    const hookIds = [];
    ...
    if (mission.hiddenOutcome.targetDamage >= 12) {
        if (target.type === "airfield") {
            ...
            maybeCreateOpportunity(state, "fighter_response_softened", "Staff believes fighter response may stay softer through the current operational window.", target.id, target.id, state.targets.filter((entry) => entry.type !== "airfield").map((entry) => entry.id), ["main_strike", "reduced_strike", "follow_up_attack"], [], hookIds, now, opportunityBeatDelta);
        ...
        if (target.type === "rail") {
            ...
            if (beneficiaryIds.length > 0) {
                maybeCreateOpportunity(state, "repair_network_slowed", "Repair traffic appears slowed. A follow-up strike may land before the damage is erased.", target.id, target.id, beneficiaryIds, ["main_strike", "follow_up_attack", "reduced_strike"], [], hookIds, now, opportunityBeatDelta);
            }
        ...
        if (target.type === "radar") {
            ...
            hookIds.push(hook.id);
            maybeCreateOpportunity(state, "warning_coordination_disrupted", "Staff believes route warning coordination may still be unsettled.", target.id, target.id, state.targets.filter((entry) => entry.type === "factory" || entry.type === "defense").map((entry) => entry.id), ["main_strike", "reduced_strike", "follow_up_attack"], ["direct"], hookIds, now, opportunityBeatDelta);
    ...
    const attacksOnTarget = state.missions.filter((entry) => entry.debriefGenerated && entry.hiddenOutcome.attackedTargetId === target.id).length;
...
function applyReconCampaignConsequences(state, recon, target, now) {
    if (recon.type === "post_strike") {
        const opportunity = state.campaign.opportunities.find((entry) => entry.state !== "resolved"
...
function getGroupEffectivenessScore(state) {
...
function getCampaignPhaseLabel(phase) {
    switch (phase) {
        case "opening":
            return "Opening";
        case "pressure":
...
function computeCampaignPhase(state) {
...
function setCampaignPhase(state, phase, now) {
...
function detectCampaignEndCondition(state) {
    ...
    if (state.campaign.currentDay > state.campaign.expectedDurationDays
...
function markCampaignResolutionPending(state, endCondition, now) {
    if (state.campaign.resolutionState !== "active") {
        return;
    }
    ...
    addCampaignEvent(state, "resolution-pending", "Resolution pending", "An end condition has been reached. Already-committed work will be allowed to finish before the campaign is judged.", now);
}
function generateCampaignEvaluation(state, now) {
    ...
    const judgment = endCondition === "directive_complete" && effectiveness >= 55
...
function maybeFinalizeCampaign(state, now) {
...
function stageReport(stage, target, riskWord, visibility, context) {
    let report;
    switch (stage) {
        case "takeoff":
            report = {
...
function generateDebrief(state, mission, beatTracker) {
...
function applyMissionEffect(state, mission, effect, beatTracker) {
    switch (effect.kind) {
...
function applyMissionEvent(state, mission, event, beatTracker) {
    if (!event.revealed) {
        event.revealed = true;
...
function completeRecon(state, recon, beatTracker) {
...
function nudgeTargetRepairs(state, elapsedMs) {
    ...
    let changed = false;
    const repairScale = clamp((state.campaign.directiveState.regionalRepairCapacity / 70) * state.campaign.profile.enemyResponse.repairBias, 0.35, 1.3);
    for (const target of state.targets) {
        ...
        const repairLift = Math.max(1, Math.round(target.hiddenRepairRate * repairScale * daySteps * profileRepairMod * (slowingOpportunity ? 0.72 : 1)));
...
function hasActiveTimedWork(state) {
...
export function startRecon(state, targetId, type, now) {
    ...
    const duration = type === "focused_followup" ? 4.5 * 60 * 1000
        : type === "post_strike" ? 3.5 * 60 * 1000
            : 2.5 * 60 * 1000;
    const resultQuality = type === "weather_route"
    ...
    const enemyAlertEffect = type === "weather_route"
        ? target.alertLevel
        : type === "post_strike"
    ...
    const recon = {
        ...
        enemyAlertEffect,
        completionApplied: false,
        hiddenAssessment: type === "weather_route"
        ...
        hiddenEvidence: type === "weather_route"
            ? target.hiddenWeatherRisk < 35
        ...
        resultQuality,
        recommendation: type === "weather_route"
            ? resultQuality === "clear"
...
function setPendingTimeAdvance(state, kind) {
    state.campaign.pendingTimeAdvanceKind = kind;
}
function getLatestPendingTime(state) {
    const candidates = [];
    for (const mission of state.missions) {
...
function getBoundedOperationalNow(state, now) {
...
function activeTargetNames(state) {
    return state.targets
        .filter((target) => target.hiddenActualCondition < 100)
        .map((target) => target.name);
}
function applyDailyStrategicDrift(state, elapsedMs, now) {
...
function applyOperationalDrift(state, elapsedMs, now, cause) {
    let changed = false;
...
function buildWaitResultNote(state, cause, elapsedMs) {
    if (cause === "passive") {
        return null;
    }
...
function buildGroupConditionText(state) {
...
function buildCampaignMomentumText(state) {
    const directive = state.campaign.directiveState;
...
function buildDirectiveStateText(state) {
...
function createConsequenceLedgerEntry(state, input) {
    const entry = {
...
function buildMissionLedgerEntry(state, mission, target) {
    const attackedTarget = getTargetById(state, mission.hiddenOutcome.attackedTargetId) ?? target;
    const lostCount = mission.hiddenOutcome.aircraftOutcomes.filter((outcome) => outcome.finalStatus === "lost").length;
    const damagedCount = mission.hiddenOutcome.aircraftOutcomes.filter((outcome) => outcome.finalStatus === "damaged").length;
    const divertedCount = mission.hiddenOutcome.aircraftOutcomes.filter((outcome) => outcome.finalStatus === "diverted").length;
    const posture = lostCount > 0 || damagedCount >= 2
...
function buildReconLedgerEntry(state, recon, target) {
    createConsequenceLedgerEntry(state, {
...
export function getOperationalRhythm(state) {
...
export function getRecentConsequenceLedger(state) {
...
function buildOpportunityRecommendation(state, opportunity) {
    ...
    return {
        recommended: createOperationalRecommendation(state, title, body, "normal", beneficiary, operationType, routeRisk, operationType === "main_strike" ? "single_pass" : "repeat_if_needed"),
        alternates: [
