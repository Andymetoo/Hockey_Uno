import { advanceCampaignDay, assignReplacementCrewMember, applyRecommendedPlan, completeAllRepairs, completeCurrentRecon, createNewGame, dismissActiveTutorialStep, formatTimestamp, getActiveMission, getActiveMissionCrewIds, getActiveRecon, getAircraftAttentionState, getAircraftAvailability, getAircraftById, getCrewAircraftLabel, getCrewById, getCrewMembersForAircraft, getCrewSeatLabel, getCurrentOperationSummary, getActiveTutorialStep, getDirectiveProgressSummary, getDisabledReasonForLaunch, getEffectiveNow, getGroundCrewPressureNote, getPrimaryUsableOpportunity, getHardUnavailablePersonnel, getLeadAircraftAssessment, getLatestDebriefMission, getLatestCompletedRecon, getMedicalActionLabel, getMedicalRecoveryLabel, getMedicalPersonnel, getOperationalRhythm, getPersonnelDecisionsForAircraft, getPlanningStaffPreview, getRecentConsequenceLedger, getReplacementCoveringMember, getReplacementPool, getRestingPersonnel, getRoleCoverageProblems, getSecondaryTargetOptions, getStaffConference, getTargetById, getTargetContextSummary, getTargetOperationalSummary, getTargetStrategicContext, keepReplacementTemporary, letCurrentWorkFinish, launchMission, loadState, markReplacementPermanent, markReplacementPermanentFromDecision, reconcileState, restoreOriginalCrewMember, removeReplacementCrewMember, resetState, saveState, setAttackDoctrine, setLeadAircraft, setLaunchMode, setOperationType, setPlanningTarget, setRouteRisk, setScheduleDelay, setSecondaryTarget, setSelectedTab, setShowHiddenValues, skipToDebrief, skipToNextReport, startRecovery, startRecon, startRepair, standDownUntilMorning, toggleAssignedAircraft, toggleStandingOrder, waitUntilNextEvent } from "./game.js";
const TABS = [
...
function renderDueTime(state, timestamp) {
...
function renderCrewRow(state, aircraftId, member, airborneIds) {
...
function renderPersonnelStatusRow(state, member) {
...
function renderStaffActionButton(recommendation) {
...
function getStaffConferencePrimaryLabel(recommendation) {
    switch (recommendation.relatedActionType) {
        case "go_debrief":
...
function renderStaffRecommendation(recommendation) {
    return `
...
function renderStaffConference(state, conference) {
    const officerLines = [
...
function renderCommandSituation(state) {
...
function buildQualitativeEvaluationSummary(state) {
...
function sanitizeCampaignSummaryText(state, text) {
    const evaluation = state.campaign.evaluation;
    if (!evaluation) {
        return text;
    }
...
function renderWhatChanged(state) {
...
function renderImmediateDecisions(state, conference) {
...
function renderCampaignRecord(state) {
                  ...
                  <div class="stack compact crew-problem-block">
                    ...
                    ${coverageProblems.map((problem) => `
                      <div class="manifest-row">
                        ...
                        <div class="button-row">
                          ${replacementPool
            .filter((member) => member.assignedAircraftId === null
        ...
        switch (action) {
            ...
            case "assign-replacement":
                if (payload) {
                    ...
                    error = assignReplacementCrewMember(state, crewMemberId ?? "", aircraftId ?? "", (role ?? "pilot"));
                }
                break;
