...
## Guided tour and return reports

The operations desk now has short, event-driven tutorial dialogs. Topics live in `tutorial.ts`: each entry owns its copy, target section, trigger, and eligibility rule. Add a topic there, then add a target marker in `App.ts` only if it needs a new place to point. The first two topics introduce the briefing and package; review, dispatch, first return, and station choices surface later. Help in the primary navigation can replay every topic. Progress, dismissal, and the station prompt's assignment are stored with the tour. Existing active tours with no tutorial record continue without surprise prompts; a new tour starts the guide. The mobile dialog is a bottom sheet, and native dialog focus and Escape handling are retained.

New return reports store a confirmed target assessment and next-assignment consequences separately from the station ledger. The visible report shows these before individual flight rows; older reports without this structure still render from their original notes. The report names diversion, grounding, persistent faults, injuries, strain, losses, earned opportunities, and the one-assignment opposition benefit when present. No committed flight result is changed when loading an older save. Mid-tour assignments 7 and 11 now use the preceding operation's result to select a follow-up objective and approach circumstance. The follow-up is recorded in both the report and next briefing; requested aircraft and target category remain intact.

The current suite has 54 passing tests. The Chrome check covers the tutorial at desktop and mobile viewport sizes, a full tour, both linked briefings, and save migration. No physical mobile or screen-reader session has been run.
...
Development and preview open `bomber_command.html`. `build:bomber` typechecks Bomber and emits `bomber-build/bomber-command.js` (a bundled classic script), `bomber-command.css`, and a standalone `bomber_command.html`. The root HTML references the compiled JS/CSS, so ordinary static hosting does not need to interpret TypeScript. Vite's Bomber-only development hook swaps those references for `main.ts` and its imported stylesheet. TypeScript remains the only implementation source.

For repository-root static hosting (including the current Firebase `public: "."` setup), run `npm run build:bomber` before serving/publishing and include **both** root `bomber_command.html` and the generated `bomber-build/` directory. Alternatively serve `bomber-build/` itself as the web root. Build artifacts are gitignored and must be regenerated in a fresh checkout; no deployment command runs as part of this build. The HTML includes a loading-failure message and embedded favicon. Never point a static page at `main.ts` or attempt to fix this by changing the `.ts` MIME type: browsers need the compiled JavaScript.

The existing `dev`, `build`, and `preview` scripts and `vite.config.ts` still belong to Ouija. Root TypeScript is explicitly no-emit and accepts `.ts` imports, preventing another accidental generation of adjacent JavaScript.
...
Replaced the old Bomber game, type model, UI and styles; removed its four adjacent JavaScript duplicates (`App.js`, `game.js`, `main.js`, `types.js`). Vite development imports `main.ts`; the static HTML loads the separately generated runtime. The old target/recon/doctrine/debug dashboards and generic seat paperwork are gone. No sibling game sources or tracked build artifacts were changed. No deployment or push was performed.
...
Static-launch regression fix: the root page was previously loading `main.ts` on static hosts, where `.ts` was served as `video/mp2t`. It now loads compiled JS/CSS; Vite alone substitutes the source entry. Verified a complete browser tour through a plain static server configured with that same `.ts` MIME mapping, Vite development, production preview, and a direct `file:///` launch in Chrome. Browser checks now also reject HTTP asset failures and TypeScript-as-video script responses. The embedded favicon avoids a separate `favicon.ico` request. This fix touches the Bomber root HTML, Bomber-specific Vite configuration, tests and this handoff; sibling games and hosting configuration remain unchanged. No deployment was performed.
