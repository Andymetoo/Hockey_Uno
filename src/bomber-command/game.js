...
export function createNewGame(now) {
    ...
    const crews = PILOT_NAMES.map((pilotName, index) => ({
        id: `crew-${index + 1}`,
        pilotName,
        experience: (index === 0 ? "veteran" : index <= 3 ? "regular" : "green"),
    ...
    const aircraft = AIRCRAFT_NAMES.map((name, index) => ({
        id: `aircraft-${index + 1}`,
        name,
    ...
    const targets = TARGET_DEFS.map((def, index) => ({
        id: `target-${index + 1}`,
        name: def.name,
        type: def.type,
        region: def.region,
...
function routeRiskMod(routeRisk) {
...
function buildMission(state, now) {
    ...
    for (const aircraftId of assignedAircraftIds) {
        ...
        const outcomeRisk = clamp((riskBase + (100 - aircraft.hiddenCondition) * 0.45 + fatigueRisk(crew.fatigue) + experienceMod(crew.experience)) / 100, 0.1, 0.96);
...
function applyFatigueRecovery(state, elapsedMs) {
