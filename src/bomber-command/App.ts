...
export function mountBomberCommand(root: HTMLElement): void {
  ...
  function report(r: Report, compact = false): string {
    ...
    const resultRows = r.results.map((f, i) => {
      ...
      return `<li><strong>${esc(s.aircraft.find(a => a.id === f.aircraft)?.name)}</strong><span>${esc(s.crews.find(c => c.id === f.crew)?.pilot)} · ${esc(f.note)}</span>${f.outcome !== 'lost' ? `<small>${f.hit ? 'Effective strike' : 'No effective strike'} · ${f.damage} condition lost · ${f.fatigue} fatigue added${f.injury ? " · specialist injured" : ""}</small>` : ''}${lead ? `<p class="flight-highlight">${esc(lead)}</p>` : ''}${more.length ? `<details data-detail="flight-${r.slot}-${i}"><summary>More from the crew debrief</summary>${more.map(line => `<p>${esc(line)}</p>`).join('')}</details>` : ''}</li>`;
    }).join('');
