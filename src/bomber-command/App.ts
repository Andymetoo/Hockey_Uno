...
import { advance, aircraftAvailable, aircraftIssue, choose, commit, createCampaign, crewAvailable, crewIssue, duration, endingText, flightFatigue, forecast, HOUR, nextMilestone, planErrors, prepareMorning, proposePlan, repairCandidates, tourEvaluation, tourMemories } from './game.ts';
...
import { autoTutorial, nextTutorial, tutorialStep, tutorialSteps, type TutorialId } from './tutorial.ts';
import { campaignToday } from './campaign.ts';
...
const opposition = (hazard: number) => hazard >= 3.5 ? 'Very heavy opposition' : hazard >= 2.5 ? 'Heavy opposition' : hazard >= 1.5 ? 'Moderate opposition' : 'Light opposition';
const threadTitles = { injury: 'A place in the crew', fault: 'Finch’s engineering record', novice: 'Learning the formation', diversion: 'The way back from a forward field' };
const settledStages = new Set(['restored', 'retained', 'recovered', 'repaired', 'home', 'overhauled', 'ashore', 'led', 'rested', 'lost', 'outgrown']);
const recordStatus = (value: string) => ({ active: 'Continuing', resolved: 'Resolved', closed: 'Closed', pending: 'Awaiting the raid', ready: 'Arranged', fulfilled: 'Fulfilled', missed: 'Objective not met', expired: 'Released' }[value] ?? value);
const entryDate = (slot: number) => slot === 0 ? 'At posting' : slot >= 14 ? 'After assignment 14' : `Before assignment ${slot + 1}`;
...
export function mountBomberCommand(root: HTMLElement): void {
  ...
  let tutorialId: TutorialId | null = null;
  let tutorialFromHelp = false, helpOpen = false, suppressAuto = false;
  let afterTutorial: 'dispatch' | null = null;
  let returnFocus = '';
  ...
  function dutyList(): string {
    const duties = s.duties?.filter(d => d.through > s.completed) ?? [];
    return duties.length ? `<ul class="duty-list">${duties.map(d => `<li><strong>${esc([s.aircraft.find(a => a.id === d.aircraft)?.name, s.crews.find(c => c.id === d.crew)?.pilot].filter(Boolean).join(' · '))}</strong><span>${esc(d.label)} · unavailable until assignment ${d.through}'s report</span></li>`).join('')}</ul>` : '';
  }
  function deskContext(decisionCount: number): string {
    const threads = s.threads.filter(t => t.status ? t.status === 'active' : !settledStages.has(t.stage));
    const stories = s.stories?.filter(a => a.status === 'active') ?? [];
    const commitments = [...campaignToday(s), ...(s.operations?.consequences.map(c => `${c.source} ${c.kind === 'escort' ? `Escort reserved for assignment ${c.dueSlot + 1}.` : `Due at assignment ${c.dueSlot}'s report.`}`) ?? [])];
    if (!s.completed && !decisionCount) return '';
    return `<section class="paper desk-summary" aria-labelledby="desk-heading"><div class="section-head"><div><p class="eyebrow">Since the last dispatch</p><h2 id="desk-heading">On the desk today</h2></div>${badge(decisionCount ? `${decisionCount} ${decisionCount === 1 ? 'matter' : 'matters'} for decision` : s.decisions.some(d => d.slot === s.completed) ? 'Instructions entered' : 'A quiet station')}</div>${s.notices.length ? `<p class="desk-latest">${esc(s.notices.at(-1))}</p>` : '<p>The roster and orders are ready for review.</p>'}${commitments.length ? `<div class="commitment-preview"><strong>Earlier decisions still in force</strong><ul>${commitments.map(c => `<li>${esc(c)}</li>`).join('')}</ul></div>` : ''}${threads.length || stories.length ? `<details data-detail="today-stories"><summary>${threads.length + stories.length} continuing ${threads.length + stories.length === 1 ? 'story' : 'stories'} · latest entries</summary>${threads.map(t => `<p><strong>${esc(threadTitles[t.family])}.</strong> ${esc(t.note)}</p>`).join('')}${stories.map(a => `<p><strong>${esc(a.title)}.</strong> ${esc(a.note)}${a.expiresSlot !== undefined ? ` <span class="small">Staff will settle this offer before assignment ${a.expiresSlot + 1} if no instruction arrives.</span>` : ''}</p>`).join('')}</details>` : ''}${dutyList()}</section>`;
  }
  function storyRecord(title: string, status: string, names: string[], milestones: { slot: number; text: string }[], note: string, key: string): string {
    return `<article class="story-record"><div class="section-head"><h3>${esc(title)}</h3>${badge(recordStatus(status), status === 'active' ? 'warning' : '')}</div>${names.length ? `<p class="story-people">${names.map(esc).join(' · ')}</p>` : ''}<p>${esc(note)}</p>${milestones.length ? `<details data-detail="${esc(key)}"><summary>Read ${milestones.length} ${milestones.length === 1 ? 'entry' : 'entries'} in the station book</summary><ol class="story-milestones">${milestones.map(m => `<li><span>${entryDate(m.slot)}</span><p>${esc(m.text)}</p></li>`).join('')}</ol></details>` : ''}</article>`;
  }
  function campaignRecord(): string {
    const commitments = s.campaign?.commitments ?? [];
    return `<section class="paper"><p class="eyebrow">Promises and consequences</p><h2>Campaign decisions</h2><p>The result of a raid and its later consequences are recorded separately.</p>${commitments.length ? commitments.map(c => `<article class="story-record"><div class="section-head"><h3>${esc(c.title)}</h3>${badge(recordStatus(c.state))}</div><ol class="story-milestones">${c.milestones.map(m => `<li><span>Assignment ${m.assignment}</span><p>${esc(m.text)}</p></li>`).join('')}</ol></article>`).join('') : s.branches.map(b => `<p><strong>Before assignment ${b.slot + 1}:</strong> ${esc(b.promise)} · ${b.fulfilled === null ? 'Pending' : b.fulfilled ? 'Fulfilled' : 'Objective remained in service'}.</p>`).join('') || '<p>No campaign direction has been chosen yet.</p>'}${s.contentVersion === 0 ? '<p class="small">This continuing tour retains its original campaign content. Begin a fresh posting for the new commitments and story families.</p>' : ''}</section><section class="paper"><p class="eyebrow">People, aircraft, and the things they carried forward</p><h2>Continuing stories</h2>${s.threads.map(t => storyRecord(threadTitles[t.family], t.status ?? (settledStages.has(t.stage) ? 'resolved' : 'active'), t.participants?.map(p => p.name) ?? [], t.milestones ?? [], t.note, `thread-${t.family}-${t.subject}`)).join('')}${s.stories?.map(a => storyRecord(a.title, a.status, a.participants.map(p => [p.name, p.specialistName].filter(Boolean).join(' / ')), a.milestones, a.note, a.id)).join('') ?? ''}${!s.threads.length && !s.stories?.length ? '<p>No continuing matter has been opened. Routine returns belong in the operational record below.</p>' : ''}${s.operations?.history.length ? `<details class="operational-decisions" data-detail="operational-decisions"><summary>Operational dilemmas and staff defaults · ${s.operations.history.length} entries</summary><ol class="story-milestones">${s.operations.history.map(h => `<li><span>${entryDate(h.slot)} · ${h.status === 'expired' ? 'Staff default' : 'Your instruction'}</span><p>${esc(h.text)}</p></li>`).join('')}</ol></details>` : ''}</section>`;
  }
  ...
  function report(r: Report, compact = false): string {
    ...
    const resultRows = r.results.map((f, i) => {
      const lead = f.details?.[0];
      const more = f.details?.slice(1) ?? [];
      return `<li><strong>${esc(f.aircraftName ?? s.aircraft.find(a => a.id === f.aircraft)?.name)}</strong><span>${esc(f.pilotName ?? s.crews.find(c => c.id === f.crew)?.pilot)} · ${esc(f.note)}</span>${f.outcome !== 'lost' ? `<small>${f.hit ? 'Effective strike' : 'No effective strike'} · ${f.damage} condition lost · ${f.fatigue} fatigue added${f.injury ? " · specialist injured" : ""}</small>` : ''}${lead ? `<p class="flight-highlight">${esc(lead)}</p>` : ''}${more.length ? `<details data-detail="flight-${r.slot}-${i}"><summary>More from the crew debrief</summary>${more.map(line => `<p>${esc(line)}</p>`).join('')}</details>` : ''}</li>`;
    }).join('');
    const debrief = r.debrief;
    const body = `<p class="report-summary">${esc(r.summary)}</p>${debrief ? `<div class="report-assessment"><h3>${r.stoodDown ? 'Station assessment' : 'What strike photographs confirm'}</h3><p>${esc(debrief.assessment)}</p></div><div class="report-next"><h3>${r.slot === 14 ? 'After the final report' : 'For the next assignment'}</h3><ul>${debrief.next.map(n => `<li>${esc(n)}</li>`).join('')}</ul></div>` : ''}${r.sent ? `<div class="accounting"><span><b>${home}</b> returned to base</span><span><b>${away}</b> landed elsewhere</span><span class="${lost ? 'caution' : ''}"><b>${lost}</b> aircraft &amp; crews lost</span></div>` : ''}${r.sent ? '<h3>Each aircraft and crew</h3>' : ''}<ul class="flight-results">${resultRows}</ul>${r.notes.length ? `<details data-detail="report-notes-${r.slot}"><summary>Station record and other changes</summary>${r.notes.map(n => `<p class="report-note">${esc(n)}</p>`).join('')}</details>` : ''}${!compact && s.phase === 'active' && !s.active ? `<div class="actions report-action">${button('go-briefing', 'Review the next assignment →', 'primary')}</div>` : ''}`;
    return compact ? `<details class="record" data-detail="report-${r.slot}"><summary><span>Assignment ${String(r.slot).padStart(2, '0')} · ${esc(r.title)}</span><span>${r.stoodDown ? 'Stood down' : `${r.hits}/${r.requested} effective`}</span></summary>${body}</details>` : `<section class="paper report" data-tutorial-target="report"><p class="eyebrow">${r.stoodDown ? 'Station' : 'Return'} report · Assignment ${r.slot}</p><h2>${esc(r.title)}</h2>${body}</section>`;
  }
  ...
  function packageOutlook(): string {
    const flights = s.plan.flights.map(f => ({ aircraft: s.aircraft.find(a => a.id === f.aircraft), crew: s.crews.find(c => c.id === f.crew) })).filter((f): f is { aircraft: Aircraft; crew: Crew } => !!f.aircraft && !!f.crew);
    if (!flights.length) return '';
    const odds = flights.map(f => forecast(s, f.aircraft, f.crew, s.plan));
    const average = (key: 'mechanical' | 'exposure' | 'accuracy') => percent(odds.reduce((sum, f) => sum + f[key], 0) / odds.length);
    return `<div class="package-outlook"><strong>Current package outlook</strong><p>Average per aircraft: ${average('accuracy')} effective bombing if the target is reached · ${average('exposure')} combat exposure · ${average('mechanical')} mechanical trouble. Estimates change with the roster and orders; they are not guaranteed outcomes.</p></div>`;
  }
  ...
  function squadron(): string {
    return `<section class="page-intro"><p class="eyebrow">The dispersal</p><h1>Aircraft &amp; their people</h1><p>Sleep reduces fatigue by twelve every six hours, down to a floor of twelve per strain level. Pressing, battle damage and exhausted flying build strain. An assignment off flying removes one level; promised leave clears all of it at that report. At 95 fatigue the medical officer grounds a crew. Injuries and diversions have their own return times.</p></section><div class="aircraft-grid">${s.aircraft.map(a => aircraftCard(a)).join('')}</div><section class="paper"><h2>The crews</h2><p>A crew can be assigned to any serviceable aircraft. Routine seat filling is automatic; specialists become your concern when their absence matters.</p><div class="aircraft-grid">${s.crews.map(crewCard).join('')}</div></section><section class="paper"><h2>Engineering &amp; recovery</h2>${dutyList()}<p>${s.support} support available. Support pays for temporary specialists, recovery transport and replacement aircraft. Effective strikes on supply objectives earn more.</p>${workList()}${s.notices.length ? `<details data-detail="station-log"><summary>Recent station entries</summary><ul class="history">${s.notices.map(n => `<li>${esc(n)}</li>`).join('')}</ul></details>` : ''}</section>`;
  }
  function tour(): string {
    return `<section class="paper"><p class="eyebrow">The fourteen-assignment tour</p><h1>A record of this posting</h1><p>${s.completed} resolved &middot; ${s.contribution} effective strikes against ${s.requested} requested so far.${!s.legacyTour ? ` ${tourEvaluation(s).objectives} objectives disrupted; ${tourEvaluation(s).commitments} of ${tourEvaluation(s).possible} campaign commitments fulfilled.` : ""}</p><p>Each disrupted objective counts once, even if extra aircraft hit it. Preserved flying crews and fulfilled campaign commitments also shape the final assessment. A dispatch or deliberate stand-down uses one assignment when its report arrives. There are no missed-day penalties: time away finishes only work already begun. A damaged or temporarily absent squadron can recover.</p>${!s.legacyTour ? `<details><summary>How the final assessment works</summary><p>Each disrupted objective counts once when confirmed strikes reach at least 65% of HQ's request. Excellent: 11 objectives, at most one aircraft or crew loss, and two fulfilled campaign commitments. Strong: 7 objectives, at most two losses, and one commitment. Mixed: 3 objectives and at most three losses. Lower results are difficult. Extra effective strikes remain in the operational tally.</p></details>` : ""}<div class="tour-track">${s.assignments.map((a, i) => `<div class="tour-slot ${i < s.completed ? 'complete' : i === s.completed ? 'current' : ''}"><b>${String(i + 1).padStart(2, '0')}</b><span>${i < s.completed ? s.reports[i].stoodDown ? 'Rest' : `${s.reports[i].hits} hits` : i === s.completed ? 'Today' : i >= 11 ? 'Final' : 'Ahead'}</span></div>`).join('')}</div>${s.phase !== 'active' ? `<p class="lead">${endingText(s)}</p>` : `<h2>Known demands</h2>${s.assignments.slice(s.completed, Math.max(s.completed + 2, 0)).map((a, i) => `<p><strong>${s.completed + i + 1}. ${esc(a.title)}</strong> · ${a.requested} aircraft · ${a.hazard >= 3 ? 'heavy' : 'lighter'} opposition</p>`).join('')}<p class="small">Final stretch: ${s.assignments.slice(11).map((a, i) => `assignment ${i + 12} requests ${a.requested}`).join('; ')}. Earlier choices can change these demands. Relief follows the final report and outstanding work.</p>`}</section>${campaignRecord()}<section class="paper"><h2>Operational record</h2>${s.reports.length ? [...s.reports].reverse().map(r => report(r, true)).join('') : '<p>The record is open. Your first assignment is waiting on Today.</p>'}${s.decisions.length ? `<details data-detail="decisions"><summary>Decisions entered in the station book</summary><ol class="history">${s.decisions.map(d => `<li>Before assignment ${d.slot + 1}: ${esc(d.text)}</li>`).join('')}</ol></details>` : ''}</section><section class="paper"><h2>Station records</h2><p>Saved automatically in this browser. A new tour archives this one first. Keep a downloaded copy if you plan to clear browser data.</p><div class="actions">${button('export', 'Download save copy')}${button('new', 'Start a new tour', 'text-button')}</div><p class="small">Posting number ${s.seed}. Bomber Command is a fictional squadron campaign inspired by B-17 operations.</p></section>`;
  }
  function dialog(): string {
    if (helpOpen) return `<dialog class="tutorial-dialog" aria-labelledby="dialog-title"><p class="eyebrow">Field guide</p><h2 id="dialog-title" tabindex="-1">Help for this tour</h2><p>Open any topic at any time. Guided prompts appear only once unless you reopen them here.</p><div class="tutorial-topics">${tutorialSteps.map(step => `<button type="button" data-tutorial-open="${step.id}">${esc(step.title)}</button>`).join('')}</div><div class="actions">${button('tutorial-close', 'Back to the desk', 'primary')}</div></dialog>`;
    if (tutorialId) {
      const step = tutorialStep(tutorialId)!;
      const next = tutorialId === 'intro' ? 'Continue to the package' : 'Got it';
      return `<dialog class="tutorial-dialog" aria-labelledby="dialog-title"><p class="eyebrow">Field guide</p><h2 id="dialog-title" tabindex="-1">${esc(step.title)}</h2><p>${esc(step.body)}</p><div class="tutorial-actions">${button('tutorial-skip', 'Skip guided prompts', 'text-button')}${button('tutorial-continue', next, 'primary')}</div></dialog>`;
    }
    if (!modal) return '';
  ...
  function render(): void {
    if (!modal && !tutorialId && !helpOpen && !blocked && !error && !suppressAuto) tutorialId = autoTutorial(s, tab);
    suppressAuto = false;
    ...
    if (modal || tutorialId || helpOpen) { const d = root.querySelector('dialog')!; if (tutorialId) root.querySelector<HTMLElement>(`[data-tutorial-target="${tutorialStep(tutorialId)?.target}"]`)?.scrollIntoView({ block: 'start' }); d.showModal(); d.querySelector<HTMLElement>('#dialog-title')?.focus(); d.addEventListener('cancel', e => { e.preventDefault(); const finished = tutorialId; if (finished) { acknowledgeTutorial(finished); tutorialId = null; modal = afterTutorial; afterTutorial = null; tutorialFromHelp = false; } else { modal = null; helpOpen = false; } suppressAuto = true; render(); if (finished && !modal) focusTutorialTarget(finished); else if (!finished) restoreFocus(); }); }
  ...
  function acknowledgeTutorial(id: TutorialId): void {
    if (tutorialFromHelp) return;
    transact(d => { if (!d.tutorial.seen.includes(id)) d.tutorial.seen.push(id); if (id === 'station') d.tutorial.stationAt = d.completed; });
  }
  function focusTutorialTarget(id: TutorialId | null): void {
    const target = id ? root.querySelector<HTMLElement>(`[data-tutorial-target="${tutorialStep(id)?.target}"]`) : null;
    if (!target) { root.querySelector<HTMLElement>('[data-action="tutorial-help"]')?.focus(); return; }
  ...
  root.addEventListener('click', e => {
    ...
    if (target.dataset.tutorialOpen) { tutorialId = target.dataset.tutorialOpen as TutorialId; tutorialFromHelp = true; helpOpen = false; suppressAuto = true; render(); return; }
    ...
    if (action === 'tutorial-help') { returnFocus = 'tutorial-help'; helpOpen = true; suppressAuto = true; render(); return; }
    if (action === 'tutorial-close' || action === 'tutorial-continue' || action === 'tutorial-skip') {
      if (action === 'tutorial-skip') transact(d => { d.tutorial.disabled = true; });
      else if (tutorialId) acknowledgeTutorial(tutorialId);
      const finished = tutorialId;
      tutorialId = null; helpOpen = false;
      if (action === 'tutorial-continue' && finished === 'intro' && !tutorialFromHelp) tutorialId = 'planning';
      else { modal = afterTutorial; afterTutorial = null; }
      tutorialFromHelp = false; suppressAuto = true; render();
      if (!tutorialId && !modal) focusTutorialTarget(finished);
      return;
    }
    if (target.dataset.decision) {
      const prompt = nextTutorial(s, 'decision', { eventKind: target.dataset.eventKind });
      if (prompt) { tutorialId = prompt; render(); return; }
      transact(d => choose(d, target.dataset.decision!, target.dataset.choice!)); render(); root.querySelector<HTMLElement>('[data-decision]:not(:disabled), [data-action="propose"]')?.focus({ preventScroll: true }); return;
    }
    if (action === 'go-briefing') { root.querySelector<HTMLElement>('[data-tutorial-target="briefing"]')?.scrollIntoView({ block: 'start', behavior: 'smooth' }); root.querySelector<HTMLElement>('[data-tutorial-target="briefing"] h1')?.focus({ preventScroll: true }); return; }
    if (action === 'prepare-morning') { transact(d => { prepareMorning(d); d.offset = Math.max(d.offset, d.now - Date.now()); }); render(); return; }
    if (action === 'review') { const prompt = nextTutorial(s, 'review'); if (prompt) { tutorialId = prompt; afterTutorial = 'dispatch'; returnFocus = 'review'; render(); return; } }
    if (action === 'review' || action === 'standdown' || action === 'new') { if (action === 'standdown' && !s.legacyTour && s.now < s.nextMorningAt) { error = 'The next operational morning has not arrived. Prepare that morning before signing another assignment.'; render(); return; } returnFocus = action; modal = action === 'review' ? 'dispatch' : action === 'standdown' ? 'standdown' : 'new'; }
