...
export function updateDebugPanel(node: HTMLElement, result: AnswerResult): void {
  ...
  const content = [
    ...
    `selected pattern id: ${result.debug.selectedPatternId || "<none>"}`,
    `selected pattern priority: ${result.debug.selectedPatternPriority ?? "<none>"}`,
    `pattern matches: ${JSON.stringify(result.debug.patternMatches, null, 2)}`,
